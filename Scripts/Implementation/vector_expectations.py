...
def data_exp_wij(v, theta, arg):
    ...
    i,j = arg
    prob_term = 2*prob_hi_is_1(i,v,theta)-1 #Modified prob term
    if type(v)==int:
        return 0
    else:
        vj = v[j]
...
def data_exp_ci(v, theta, arg):
    ...
    i = arg
    p_hi_term = 2*prob_hi_is_1(i, v, theta)-1
...
def model_exp_wij(v,theta, arg):
    ...
    i,j=arg
    prob_term = 2*prob_hi_is_1(i, v, theta)-1 #Converted into new basis.
    vj_term = v[j]
...
def model_exp_ci(v, theta, arg):
    ...
    new_basis_prob_term = 2*prob_term - 1
