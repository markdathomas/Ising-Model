...
def prob_v_given_theta(all_visible, all_hidden, v, theta,Z):
    ...
    prod_2 = 1
    for i in range(n_hidden):
        sum_1 = 0 #Sum term in equation 5 adds, so start with 0.
        for j in range(m_visible):
            sum_1+=w[i][j]*v[j] #Exponent in equation 5 sum.
        prod_2 *= (1+np.exp(c[i]+sum_1)) #Second product term in equation 5.
    p = prod_1*prod_2/Z #Multiply the three distinct terms to get p.
    """
