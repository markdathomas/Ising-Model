...
def prob_hi_is_1(i, v_vec,theta):
    ...
    if type(v_vec)==int:
        ...
        prob = sigmoid(2*marg_number) #Prob is sigmoid of the sum, factor of 2 for (-1,+1) basis
        return prob
...
def cdk(k, theta):
    ...
    #Convert the binary vector into the (-1,+1) basis for spin.
    new_training_v_11 = np.array([2*s-1 for s in new_training_v_01])
...
def batch_generation(k, theta, batch_size):
    ...
    for i in range(batch_size):
        vk,hk = cdk(k, theta)
        entry = vk #Only the visible vector is stored and used for the batch.
        S.append(entry)
